    ...
    B::ManagerRef: HasWorkers,
{
    ...
    if !cli.parallel {
        let mut count = 0;
        let mut next_visualization = count;
        while let Some(task) = executor.0.pop_front() {
            ...
            if cli.build_visualization && next_visualization == count {
                let funcs = if cli.build_visualization_show_all {
                    executor
                        .0
                        .iter()
                ...
                mref.with_manager_shared(|manager| {
                    let _ = Visualizer::new().add("DD", manager, funcs).serve();
                });
                next_visualization = count + 1 + cli.build_visualization_skip;
            }
            count += 1;
