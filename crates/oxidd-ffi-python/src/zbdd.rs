...
impl ZBDDManager {
    ...
    ///     None
    #[pyo3(
        signature = (/, path, functions, variables, dd_name= "zbdd",  ascii=true),
    ...
    fn dump_dddmp_file<'py>(
        ...
        let variable_names = variables
            ...
            .map(|(_v, s)| s.to_string_lossy())
            .collect::<Vec<_>>();
        println!("variables: [{}]", variable_names.iter().map(|n| format!("{}, ", n)).collect::<String>());
    ...
    ///     None
    #[pyo3(
        signature = (/, functions, variables, dd_name= "zbdd",  port=None),
